...
class Controllerpaymentpaymentsensehosted extends Controller {
	public function index() {
		...
		$this->load->model('payment/paymentsense_hosted');
		...
		$data['fields']['CallbackURL'] = (((HTTPS_SERVER) ? HTTPS_SERVER : HTTP_SERVER) . 'index.php?route=payment/paymentsense_hosted/callback');
		...
		$data['fields']['ServerResultURL'] = (((HTTPS_SERVER) ? HTTPS_SERVER : HTTP_SERVER) . 'index.php?route=payment/paymentsense_hosted/postback');
		...
		{
			return $this->load->view($this->config->get('config_template') . '/template/payment/paymentsense_hosted.tpl', $data);
		} 
		else 
		{
